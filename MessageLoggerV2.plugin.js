  ...
  start() {
    let onLoaded = () => {
      try {
        ...
        this.stop();
        XenoLib.Notifications.error(`[${this.getName()}] Failed to start! Check console (CTRL + SHIFT + I, click console tab) for more error info.`, { timeout: 0 });
      }
    };
    ...
    if (global.BdApi && typeof BdApi.getPlugin === 'function' /* you never know with those retarded client mods */) {
      ...
      if (isOutOfDate(iXenoLib, '1.3.17')) XenoLibOutdated = true;
      if (isOutOfDate(iZeresPluginLibrary, '1.2.14')) ZeresPluginLibraryOutdated = true;
    }
    ...
    return [
      {
        title: 'Fixed',
        type: 'fixed',
        items: ['Clear log button works once more. Again.']
      }
    ];
  }
  initialize() {
...
*/
  ...
  patchMessages() {
    ...
    const MemoMessage = ZeresPluginLibrary.WebpackModules.find(m => m.type && m.type.toString().search(/\w\.useContextMenuMessage\)\(\w,\w,\w\),\w=\(0,\w\.useClickMessage\)\(/) !== -1);
          ...
          ZeresPluginLibrary.DiscordModules.React.createElement(
            ...
            _ =>
              ZeresPluginLibrary.DiscordModules.React.createElement(
                ...
                parseContent({ channel_id: props.message.channel_id, mentionChannels: props.message.mentionChannels, content: edit.content }).content,
                noSuffix
                  ? null
  ...
  patchModal() {
    ...
    this.createModal.confirmationModal = class ConfirmationModal extends ZeresPluginLibrary.DiscordModules.ConfirmationModal {
      ...
      handleSubmitEx(e) {
        ...
        else return this._submitModal(e);
      }
      handleCloseEx(e) {
    ...
    if (profImg) {
      ...
      timestampEl.addEventListener('contextmenu', e => {
        ...
        ZeresPluginLibrary.DCM.openContextMenu(
          e,
          ZeresPluginLibrary.DCM.buildMenu([
            {
              type: 'group',
              items: [
                {
  ...
  handleContextMenu(thisObj, returnValue) {
    ...
    // console.log(thisObj, returnValue);
    const type = thisObj.props.type;
    const newItems = [];
    const addElement = (label, callback, options = {}) => {
      newItems.push(
    ...
    const handleWhiteBlackList = id => {
      ...
      addElement(`${notifIdx === -1 ? 'Add To' : 'Remove From'} Notification Blacklist`, () => {
    ...
    if (type === 'NATIVE_IMAGE') {
      ...
      if (thisObj.props.src.startsWith('data:image/png')) {
        ...
        if (!matched) {
          matched = thisObj.props.src.match(/localhost:7474.*#(\d+),(\d+)/);
          isCached = true;
        }
      }
      if (!matched) return;
      ...
      if (record) {
        ...
        if (record.delete_data) {
          ...
          addElement(`${idx !== -1 ? 'Add' : 'Remove'} Deleted Tint`, () => {
          ...
          if (record.edits_hidden) {
            ...
            if (target) {
              ...
              if (target) {
                ...
                if (this.editModifiers[messageId]) {
                  addElement(`${this.editModifiers[messageId].noSuffix ? 'Show' : 'Hide'} (edited) Tag`, () => {
                    this.editModifiers[messageId].noSuffix = true;
                    ZeresPluginLibrary.DiscordModules.Dispatcher.dispatch({ type: 'MLV2_FORCE_UPDATE_MESSAGE_CONTENT', id: messageId });
                  });
                  addElement(`Undo Show As Message`, () => {
                    delete this.editModifiers[messageId];
                    ZeresPluginLibrary.DiscordModules.Dispatcher.dispatch({ type: 'MLV2_FORCE_UPDATE_MESSAGE_CONTENT', id: messageId });
                  });
                } else if (typeof editNum !== 'undefined' && editNum !== null) {
                  addElement('Show Edit As Message', () => {
      ...
      returnValue.props.children.push(
        ZeresPluginLibrary.DiscordModules.React.createElement(this.ContextMenuGroup, {
          children: [ZeresPluginLibrary.DiscordModules.React.createElement(this.SubMenuItem, { label: this.settings.contextmenuSubmenuName, render: newItems, invertChildY: true })]
        })
      );
      return;
    }
