...
var DetailedServerTooltips = (() => {
  /* Setup */
  const config = {
    main: 'index.js',
    info: { name: 'DetailedServerTooltips', authors: [{ name: 'Lighty', discord_id: '239513071272329217', github_username: 'LightyPon', twitter_username: '' }], version: '1.0.5', description: 'Displays a more detailed server tooltip, containing its image, owner tag, creation date, join date, member count, channel count, role count, region and if the server is partenered.', github: 'https://gitlab.com/_Lighty_', github_raw: 'https://_lighty_.gitlab.io/bdstuff/plugins/DetailedServerTooltips.plugin.js' },
    changelog: [{
      title: 'sad',
      type: 'added',
  ...
  const buildPlugin = ([Plugin, Api]) => {
    const { Settings, Utilities, WebpackModules, DiscordModules, ReactTools, ReactComponents, Patcher, PluginUtilities } = Api;
    const { React, ChannelStore, UserStore, MemberCountStore, GuildStore } = DiscordModules;
      ...
      render() {
        ...
        const fixAlignment = () => {
          ...
          if (bcr.top < 0) {
            ...
            ref.parentElement.parentElement.style.top = window.innerHeight - bcr.height + 'px';
          }
        ...
        return React.createElement(
          ...
          this.props.settings.showName && React.createElement(DataLine, common, this.props.guild.name),
          this.props.settings.showOwner && React.createElement(DataLine, common, `Owner: ${owner ? owner.tag : 'unknown'}`),
          this.props.settings.showCreated && React.createElement(DataLine, common, `Created: ${createdAt.toLocaleDateString()}, ${createdAt.toLocaleTimeString()} (${Math.round(Math.abs(createdAt.getTime() - new Date().getTime()) / 86400000)} days ago)`),
          this.props.settings.showJoined && this.props.guild.joinedAt && createdAt.toString() !== this.props.guild.joinedAt.toString() && React.createElement(DataLine, common, `Joined: ${this.props.guild.joinedAt.toLocaleDateString()}, ${this.props.guild.joinedAt.toLocaleTimeString()} (${Math.round(Math.abs(this.props.guild.joinedAt.getTime() - new Date().getTime()) / 86400000)} days ago)`),
          this.props.settings.showMembers && React.createElement(DataLine, common, `${MemberCountStore.getMemberCount(this.props.guild.id) || 0} members`),
          this.props.settings.showChannels && React.createElement(DataLine, common, `${Object.values(ChannelStore.getChannels()).filter(e => e.guild_id === this.props.guild.id).length} channels`),
          this.props.settings.showRoles && React.createElement(DataLine, common, `${Object.keys(this.props.guild.roles).length} roles`),
          this.props.settings.showRegion && React.createElement(DataLine, common, this.props.guild.region),
      ...
      async patchGuildIcon(promiseState) {
        ...
        */
        ...
        const patchFunc = e => {
          ...
          if (e._DSTTActive) {
            ret2.props.text = React.createElement(CustomTooltip, { guild: e.guild, forceUpdate: () => ReactTools.Reflect(document.querySelector(`.${WebpackModules.getByProps('tooltip').tooltip.split(' ')[0]} > div:last-child`)).forceUpdate(), settings: this.settings.display });
